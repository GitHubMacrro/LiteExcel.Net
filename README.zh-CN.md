...
轻量级 xlsx 读写库，零第三方依赖，AOT 友好。**版本 2.1.3**
...
- **全功能**：读/写、样式（4 级优先级）、合并单元格、自动筛选、行高/列宽、批注、数据验证、追加、Stream、List\<T\>/DataTable 便利 API
...
| API | AOT |
|---|---|
| `SheetData` / `DataTable` 读写 | ✅ 安全（无反射） |
| `List<T>` 读写（`Read<T>` / `Write<T>`） | ⚠️ 标注 `[RequiresUnreferencedCode]`，AOT 编译时提示警告 |
